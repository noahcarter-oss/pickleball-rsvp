...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const confirmed = await supabaseGet(
      `invitees?select=*&game_id=eq.${encodeURIComponent(gameId)}&status=eq.confirmed&email=not.is.null&order=responded_at.asc`,
    );

    if (!game.length) return res.status(404).json({ error: "Game not found" });
    const recipients = confirmed.map((invitee) => invitee.email).filter(Boolean);
    if (!recipients.length) return res.status(400).json({ error: "No confirmed players have email addresses" });

    const activeGame = game[0];
    const names = confirmed.map((invitee) => invitee.name).join(", ");

    await sendEmail({
      to: recipients,
      subject: "Pickleball reminder",
      html: emailTemplate({
        title: "Pickleball reminder",
...
function emailTemplate({ title, body, note }) {
  return `
    <div style="font-family: Arial, sans-serif; max-width: 560px; margin: 0 auto; color: #202522;">
      ...
      <p style="font-size: 15px; color: #667169; margin: 0;">${escapeHtml(note)}</p>
    </div>
  `;
}
