...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const recipients = invitees.filter((invitee) => invitee.email);
    ...
    await Promise.all(
      recipients.map((invitee) => {
        const rsvpUrl = `${baseUrl}/?game=${activeGame.id}&rsvp=${encodeURIComponent(invitee.token)}`;
        return sendEmail({
          to: invitee.email,
          subject: "Pickleball RSVP",
          html: emailTemplate({
            title: `Pickleball, ${invitee.name}?`,
